...
RC PredicatePushdownRewriter::rewrite(std::unique_ptr<LogicalOperator>& oper, bool& change_made) {
    ...
    if (child_oper->type() == LogicalOperatorType::TABLE_GET) {
        auto child_logical_oper = static_cast<TableGetLogicalOperator*>(child_oper.get());
        std::vector<std::unique_ptr<Expression>> pushdown_exprs;
        pushdownPredicate(oper, change_made, child_logical_oper, pushdown_exprs);
        ...
        auto child_logical_oper = static_cast<JoinLogicalOperator*>(child_oper.get());
        std::vector<std::unique_ptr<Expression>> pushdown_exprs;
        pushdownPredicate(oper, change_made, child_logical_oper, pushdown_exprs);
        if (!pushdown_exprs.empty()) {
            change_made = true;
...
RC PredicatePushdownRewriter::pushdownPredicate(std::unique_ptr<LogicalOperator>& oper,
                                                bool& change_made,
                                                LogicalOperator* child_logical_oper,
                                                std::vector<std::unique_ptr<Expression>>& pushdown_exprs) {
    RC rc = RC::SUCCESS;
