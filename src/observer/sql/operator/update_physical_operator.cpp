...
RC UpdatePhysicalOperator::next()
{
  ...
  while (RC::SUCCESS == (rc = child->next())) {
    ...
    if (table_->project_physical_oper_ != nullptr) {
      ViewTuple            *view_tuple = static_cast<ViewTuple *>(tuple);
      std::vector<Record *> records;
      view_tuple->get_record(records);
      int                index;
      ...
      for (const auto &pair : update_map_) {
        ...
        const FieldMeta   *field        = table_->table_meta().field(field_name.c_str());
        const Table       *update_table = table_->field_link_to_table_.at(field);
        ...
        for (Record *record : records) {
          rc = const_cast<Table *>(update_table)->get_record(record->rid(), table_record);
          if (rc == RC::SUCCESS) {
            break;
          }
        }
        if (rc != RC::SUCCESS) {
          LOG_ERROR("fetch record err");
          return rc;
        }
        ...
        rc = trx_->update_record(
            const_cast<Table *>(update_table), table_record, std::move(offsets), std::move(indexs), std::move(values), std::move(lens));
        if (rc != RC::SUCCESS) {
      ...
      rc = trx_->update_record(
          table_, *record, std::move(offsets), std::move(indexs), std::move(values), std::move(lens));
      if (rc != RC::SUCCESS) {
        LOG_WARN("failed to update record: %s", strrc(rc));
        sql_debug("failed to update record: %s", strrc(rc));
        sql_debug("test");
