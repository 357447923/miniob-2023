...
{
public:
  // ProjectLogicalOperator(std::vector<std::unique_ptr<Expression>> &fields);
  ...
  const std::vector<Expression *> *schemas() const
  {
    return schemas_;
  }
...
private:
  ...
  //! 不过现在简单处理，就使用字段来描述
  //! 从SelectStmt中获取的，并且由SelectStmt管理
  const std::vector<Expression *> *schemas_;
};
