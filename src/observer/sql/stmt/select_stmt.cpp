    ...
    const std::unordered_map<std::string, Table *> &parent_table_map, Stmt *&stmt)
{
    ...
    if (common::is_blank(relation_attr.relation_name.c_str()) &&
      ...
      if (relation_attr.type == FUNC_NONE) {
        ...
        if (field_meta->type() == AttrType::CHARS &&
            (relation_attr.type == FUNC_SUM || relation_attr.type == FUNC_AVG)) {
        ...
        Expression *aggr_expr =
            new AggrFuncExpr(relation_attr.type, new FieldExpr(table, field_meta, relation_attr.type));
        aggr_expr->set_alias(alias);
        query_expressions.push_back(aggr_expr);
      }
    }
  ...
  if (select_sql.join_conditions.size() != 0) {
    for (std::vector<ConditionSqlNode> join_condition : select_sql.join_conditions) {
      rc = FilterStmt::create(db,
          ...
          tables,
          join_condition.data(),
          static_cast<int>(join_condition.size()),
  ...
  if (!select_sql.havings.empty()) {
    rc = HavingStmt::create(
        db, default_table, &table_map, tables, select_sql.havings.data(), select_sql.groups.size(), having_stmt);
    if (rc != RC::SUCCESS) {
...
void SelectStmt::move_to(SelectStmt &select_stmt) {
  select_stmt.query_expressions_.swap(query_expressions_);
  select_stmt.filter_stmt_ = filter_stmt_;
  filter_stmt_ = nullptr;
  select_stmt.groupby_stmt_ = groupby_stmt_;
  groupby_stmt_ = nullptr;
  select_stmt.inner_join_filter_stmts_.swap(inner_join_filter_stmts_);
  select_stmt.having_stmt_ = having_stmt_;
  having_stmt_ = nullptr;
  select_stmt.orderby_stmt_ = orderby_stmt_;
  orderby_stmt_ = nullptr;
  select_stmt.orderby_for_groupby_ = orderby_for_groupby_;
  orderby_for_groupby_ = nullptr;
  select_stmt.tables_.swap(tables_);
}