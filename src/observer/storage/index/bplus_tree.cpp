              ...
              int leaf_max_size /* = -1 */)
{
  ...
  key_comparator_.init(indexType, attr_id, attr_type, attr_length);
...
RC BplusTreeHandler::open(const char *file_name)
{
  ...
  for (int i = 0; i < file_header_.attr_num; i++) {
    ...
    attr_length.push_back(file_header_.attr_id[i]);
  }

  key_comparator_.init(file_header_.index_type, attr_id, attr_type, attr_length);
  key_printer_.init(attr_type, attr_length);
...
MemPoolItem::unique_ptr BplusTreeHandler::make_key(const char *user_key, const RID &rid)
{
  ...
  int pos = 0;
  for (int i = 0; i < file_header_.attr_num; i++) {
    memcpy(composite_key_data + pos, user_key + pos, file_header_.attr_length[i]);
    pos += file_header_.attr_length[i];
  }
  memcpy(composite_key_data + pos, &rid, sizeof(rid));
  return key;
}
