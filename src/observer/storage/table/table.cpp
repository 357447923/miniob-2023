...
RC Table::update_record(Record& record, std::vector<int> offsets, std::vector<int> indexs, std::vector<Value> values, std::vector<int> lens) {
    ...
    // 如果索引删除成功了，保存修改之前的 record 方便回滚时插入索引
    if (rc == RC::SUCCESS) {
    ...
    // 如果插入索引成功，将插入的记录保存起来，方便回滚时找到索引并删除
    if (rc == RC::SUCCESS) {
...
RC Table::rollback_update() {
    RC rc = RC::SUCCESS;
    // 删除新插入的索引
    bool b = false;
    for (Record& record : inserted_records) {
...
RC Table::create_index(Trx* trx, const std::vector<FieldMeta> field_metas, const char* index_name, IndexType indexType) {
    ...
    RC rc = new_index_meta.init(index_name, indexType, field_names);
    if (rc != RC::SUCCESS) {
        LOG_INFO("Failed to init IndexMeta in table:%s, index_name:%s, field_name:%s", name(), index_name, field_names);
        return rc;
    }
